...
class UCSRepoPool5(_UCSRepo):
    ...
    def deb(self, server, type="deb"):
        ...
        >>> r=UCSRepoPool(major=5,minor=1,patchlevel=0)
        >>> r.deb('https://updates.software-univention.de/')
        'deb https://updates.software-univention.de/ ucs510 main'
        >>> r=UCSRepoPool(major=5,minor=1,patchlevel=0,errata=True)
        >>> r.deb('https://updates.software-univention.de/')
    ...
    def path(self, filename=None):
        ...
        >>> UCSRepoPool(major=5,minor=1,patchlevel=0).path()
        'dists/ucs500/InRelease'
        >>> UCSRepoPool(major=5,minor=1,patchlevel=0,errata=True).path()
...
class UniventionUpdater(object):
    ...
    def get_releases(server):
        try:
            ...
            if server.proxy_handler.proxies:
                ud.debug(ud.NETWORK, ud.WARN, 'Maintenance information malformed, blocked by proxy?')
            raise StopIteration
        except KeyError as exc:
            ud.debug(ud.NETWORK, ud.ERROR, 'Maintenance information missing releases: %s' % (exc,))
            raise StopIteration
	...
	result = [UCSRepoPool5(version['major'], version['minor'], version['patchlevel']).deb(self.server)]
        for component in components:
            repos = []  # type: List[str]
            try:
